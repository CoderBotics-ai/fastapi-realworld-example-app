...
from app.api.dependencies.authentication import get_article_by_slug_from_path
from app.models.domain.responses import ArticleInResponse, ArticleForResponse
...
async def delete_article_by_slug(
    ...
    articles_collection: Collection = articles_repo._collection
...
async def update_article_by_slug(
    ...
    await articles_repo.collection.update_one({"_id": current_article.id}, {"$set": article_data})
    article = await articles_repo.collection.find_one({"_id": current_article.id})
    return ArticleInResponse(article=ArticleForResponse.from_orm(article))
